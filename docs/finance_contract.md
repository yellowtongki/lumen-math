...
# ③ 학생 경제 · 세금 준비 · 시뮬레이션 (v19-10)

v19-9에 이어 같은 「💳 수강료·경영」 화면에 칩 두 개를 더한다: `👥 학생 경제` · `🧮 세금·시뮬레이션`.

## 10. 쓸 수 있게 된 자료 (2026-09-17 실측)
- `pay_bills` **324건** · `pay_payments` **286건** (2024-09-17~2026-09-17, 2년) · `pay_students.byName` **42명**(퇴원생 포함) · `pay_monthly` 24달 중 **14달 실자료(2025-08~2026-09)**.
- 수집기는 긴 기간을 90일 조각으로 나눠 받는다(`dateChunks`). 빈 결과로는 덮어쓰지 않는다.

## 11. 👥 학생 경제
- **학생 수 추이**: 달마다 「그 달에 청구서를 받은 사람 수」를 선 그래프(14달). 옆에 이번 달 재원 수(명단 기준).
- **입회·퇴원**: 어떤 이름이 처음 청구된 달 = 입회, 마지막 청구 뒤 2달 넘게 없으면 퇴원으로 본다. 달마다 +입회 −퇴원 막대(입회 `#1d6fe8` · 퇴원 `#d97706`, 검증된 색).
- **이탈률**: 최근 6달 평균 (퇴원 ÷ 그달 인원). 한 줄 숫자.
- **평균 재원 기간**: 퇴원한 사람들의 (마지막−처음) 달 수 평균. 아직 다니는 사람은 뺀다.
- **학생 1명의 평생 가치**: 1인 평균 월 수납액 × 평균 재원 기간. 「지금 학생 한 명이 앞으로 학원에 남기는 돈」이라고 풀어 쓴다.
- **누적 납부 표**: 이름(명단에 있으면 실명, 없으면 결제선생 이름) · 첫 납부 · 마지막 납부 · 재원 달수 · 누적 납부액 · 형제 표시. 누적 많은 순, 「퇴원」 꼬리표. 상위 15명 + 「모두 보기」.
- 퇴원생은 이 칸에서는 **보인다**(과거 자료이므로). 다만 현재 재원 표(v19-8 학생별 납부)에는 여전히 안 나온다. `check_withdrawn_leak.js`가 검사하는 항목은 건드리지 않는다 — 이 칸은 결제선생 이름 기준이고 명단의 퇴원생 정보를 새로 노출하지 않는다.

## 12. 🧮 세금 준비
- 해 고르기(2025·2026). 표: 카드 매출 · 계좌이체 · 현금 · 합계(그 해 `sales.byMethod` 합), 비용 합계(고정비+변동비, `fin_cfg`로 계산한 달들만), 소득 추정(매출−비용).
- 달별 표(12줄): 매출 · 고정비 · 변동비 · 순이익. 「⬇ 엑셀(CSV)」로 내려받기 — 세무사 전달용.
- 학원은 **면세사업자**라 부가세 신고가 없고, 2월 사업장현황신고·5월 종합소득세에 쓴다는 한 줄 안내.
- 비용을 안 적은 달은 「비용 미입력」으로 표시하고 합계에서 뺀다.

## 13. 🎯 시뮬레이션 (세금 칸 아래)
기준: 이번 달 명단 규칙 합계·최근 3달 평균 수납률·고정비·변동비.
- 칸 네 개(범위 슬라이더 또는 −/+ 단추): **수강료 인상 %**(0~20) · **학생 수 증감**(−5~+10, 학년 고르기) · **고정비 증감 원** · **수납률 %**.
- 바꾸면 바로 아래 세 줄이 다시 계산: 예상 월 매출 · 예상 순이익(지금 대비 ±) · 손익분기 학생 수.
- 미리 정해 둔 단추 네 개: 「수강료 5% 인상」 「중등 3명 더」 「고등 1명 퇴원」 「매쓰플랫 해지(−220,000)」 — 누르면 칸이 그 값으로 채워진다.
- 결과는 저장하지 않는다(그때그때 보기).

## 14. 검증
- 문법 · `check_withdrawn_leak.js` 10/10.
- 하네스 `verify_v1910.js`: fixture에 2년치 비슷한 자료(달 14개, 이름 8명 중 2명은 퇴원 꼴) → 학생 수 추이 점 개수, 입회·퇴원 수, 이탈률·평균 재원·평생 가치 수치, 누적 표 정렬·퇴원 꼬리표, 세금 표 해별 합계·CSV 글, 시뮬레이션 네 칸을 바꿨을 때 순이익 변화와 미리 정해 둔 단추. v19-9 회귀(`verify_v199.js` 기대 버전만 바꿔).
