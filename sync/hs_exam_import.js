#!/usr/bin/env node
/* ═══════════════════════════════════════════════════════════════════
 * 고등학교 기출 → 매쓰플랫 학교별 폴더 일괄 등록  v1  (2026-08-28)
 * ═══════════════════════════════════════════════════════════════════
 *
 * 【무엇을 하나요?】
 *   수학비서에 사 두신 학교 기출(소사고·범박고 등)을 매쓰플랫 「기타 학습자료」로
 *   등록하고, 학교별 폴더(예: 「범박고 기출」)에 학습지로 담습니다.
 *   학년은 폴더로 나누지 않습니다 — 학습지 제목에 「1학년 2026년 1학기 기말」이
 *   들어가서 이름순 정렬이 곧 학년·연도순이고, 매쓰플랫 폴더가 최대 20개뿐이라
 *   학교당 하나여야 학교를 계속 늘릴 수 있습니다.
...
function planOf(row) {
  ...
  /* 폴더는 <b>학교별 한 개</b>. 학년으로 또 나누지 않는 이유 (2026-08-28 원장님과 결정):
   *   ① 제목에 이미 「1학년 2026년 1학기 기말」이 들어가 이름순 정렬이 곧 학년·연도순
   *   ② 매쓰플랫 폴더는 최대 20개뿐 — 학교당 1개여야 학교를 계속 늘릴 수 있다
   *   ③ 나중에 고3을 추가할 때 폴더를 새로 만들 필요가 없다 */
  return { trie, subject: canon, gradeLabel: `고 ${grade}-${sem}`, title,
    mylist: `${row.school} 기출`, sortKey: `${row.school}${grade}${year}${sem}${term === '중간' ? 1 : 2}` };
}
