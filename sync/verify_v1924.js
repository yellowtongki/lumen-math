...
const FILE=process.argv[2]||'lumen_v19-24.html';
const WANT=process.argv[3]||'v19-24';
...
ok('폰 칩도 다섯 개로 맞췄다', /\['all','한눈에'\],\['bk','📗 교재'\],\['ws','📄 학습지'\],\['mfstu','👤 학생'\],\['stu','⚠️ 확인'\]/.test(src));
...
(async()=>{
  ...
  const all=await p.evaluate(()=>{
    ...
    return { len:t.length, tab:MF.tab,
      ...
      tiles:['오늘 푼 문항','이번 주 문항','평균 정답률','확인 필요','반영 대기','멈춘 교재'].every(x=>t.indexOf(x)>=0),
  ...
  const st=await p.evaluate(()=>{
    ...
    return { len:t.length, rows:C.querySelectorAll('[onclick^="mfStu"]').length, model:rows.length,
             head:['학생','📗 교재','📄 학습지','확인 필요','반영 대기'].every(x=>t.indexOf(x)>=0),
  ...
  const filt=await p.evaluate(()=>{
    mfStu(null); mfOnly('chk'); const a=mfStuRows(); const allChk=a.every(r=>r.chk>0);
    ...
    return { chkN:a.length, allChk, deadN:d.length, allDead, m };
  });
  ok('⚠️ 확인 필요만 거르개가 맞다', filt.allChk===true, filt.chkN+'명');
